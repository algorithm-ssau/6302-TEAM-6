...
class TelegramBot:
    ...
    async def process_summarization(self, chat_id, additional_context, context: ContextTypes.DEFAULT_TYPE):
        ...
        system_prompt = (
            ...
            "Нельзя придумывать что-то, чего не было в исходном тексте. При составлении ответа ты должен опираться "
            "только на ту информацию, которую получил от пользователя (исходный текст)."
        )
